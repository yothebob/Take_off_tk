...
def total_parts_sc(_array):
    ...
    part_names = ['Total LF','TR','BR','Pocket infill','Flat infill','SPE1', 'SPE2','PEU','p421','p422','fmpbs1','fmpbs2','Halfen','Corner Halfen','L bracket','pt-420','PVI','spacer 100','spacer 200','rcb1','rcb2','rcb screws','pc','gvs bot','gvs top','spl200','int90','int135','end plate','EP screw','SDS bag','NC/CW','lags',' Kwikset grout']
...
#---------------------------------------------------------     
    ...
    part_list = [job_lf,tr,br, pocket_infill,flat_infill,spe1, spe2,peu,p421,p422,fmpbs1,fmpbs2,halfen,corner_halfen,l_bracket,picket,pvi,spacer100,spacer200,rcb1,rcb2,rcb_screw,pc,gvs_bot,gvs_top,spl200,int90,int135,end_plate,ep_screw,sds_screw,nccw,lags,grout]
...
str_br.set('BR200')
str_mount.set('BP')
str_infill.set('Picket')

section = Text(width=80,height=25)
section.grid(row=1,column=0)
enter_button = Button(text='Run (Sections ONLY)',command=find_des).grid(row=8,column=0)
sect_label= Label(text='Sections: (10x10)').grid(row=0,column=0)
